...
public class RequestHandler {
    ...
    private static final String useProxy = UtilProperties.getPropertyValue("url", "proxy.urls", "false");
    ...
    /**
     * Check the proxy.properties file for a key match with url, replace with corresponding value.
     *
     * @param url
     * @return
     */
    public static String proxyUrl(String url, HttpServletRequest request) {
        // Dont bother checking if we've turned proxied urls off.
        if(useProxy.equalsIgnoreCase("true")) {
            String cleanUrl = url;

            // Clear out the domain, and any jsessionid nonsense
            if(cleanUrl.startsWith("http://")) {
                cleanUrl = cleanUrl.substring(cleanUrl.indexOf("/", 8));
            }
            if(cleanUrl.startsWith("https://")) {
                cleanUrl = cleanUrl.substring(cleanUrl.indexOf("/", 9));
            }
            if(cleanUrl.indexOf(";jsessionid")>0) {
                cleanUrl = cleanUrl.substring(0, cleanUrl.indexOf(";jsessionid"));
            }
            // First replace '=' with '!' (Needed to do this for properties keys)
            String bangUrl = cleanUrl.replace("=", "!").replace("&#x3d;", "!").replace(":", "#").replace("&#x2f;", "/").replace("&#x7e;", "~");

            // Find the proxy
            String proxiedUrl = UtilProperties.getPropertyValue(getProxyFile(request), bangUrl);
            // Replace the cleanUrl in url with the proxy (as long as we found a proxy)
            ...
            if(UtilValidate.isNotEmpty(proxiedUrl)) {
                ...
                url = url.replace(cleanUrl, proxiedUrl);
            } else {
                url = cleanUrl;
            }
    ...
    public static String getProxyFile(HttpServletRequest request) {
        if(request != null) {
            String controlPath = (String) request.getAttribute("_CONTROL_PATH_");
            if(UtilValidate.isNotEmpty(controlPath) && controlPath.contains("envelopes")) {
                return "proxy";
            } else if(UtilValidate.isNotEmpty(controlPath) && controlPath.contains("bags")) {
                return "bags-proxy";
            } else if(UtilValidate.isNotEmpty(controlPath) && controlPath.contains("folders")) {
                return "folders-proxy";
            } else if(UtilValidate.isNotEmpty(controlPath) && controlPath.contains("bigname")) {
