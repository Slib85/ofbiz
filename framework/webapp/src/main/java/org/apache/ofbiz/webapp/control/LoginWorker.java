...
public class LoginWorker {
    ...
    public static String login(HttpServletRequest request, HttpServletResponse response) {
        ...
        /*
            ENV social login data
            We will spoof an empty password just to pass null checks
        */
        if(UtilValidate.isNotEmpty(token)) {
            Map<String, Object> socialUser = null;
            try {
                socialUser = ((LocalDispatcher) request.getAttribute("dispatcher")).runSync("getSocialUserLogin", UtilMisc.<String, String>toMap("token", token));
            } catch (Exception e) {
                Debug.logWarning(e, "", module);
            }

            if(UtilValidate.isNotEmpty(ServiceUtil.isSuccess(socialUser))) {
                request.getSession().setAttribute("externalUserLogin", (GenericValue) socialUser.get("externalUserLogin"));
                username = (String) socialUser.get("username");
        ...
        try {
            ...
            result = dispatcher.runSync("userLogin", UtilMisc.toMap("login.username", username, "login.password", password, "token", token, "visitId", visitId, "locale", UtilHttp.getLocale(request)));
    ...
    public static String checkPassword(HttpServletRequest request, String userName, String password) {
		HttpSession session = request.getSession();
		LocalDispatcher dispatcher = (LocalDispatcher) request.getAttribute("dispatcher");
		Map<String, Object> result = null;
