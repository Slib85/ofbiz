					...
					<form>
						...
						<#list payments as payment>
							<div<#if payment.paymentMethodTypeId?exists && (payment.paymentMethodTypeId == "EXT_AMAZON" || payment.paymentMethodTypeId == "EXT_PAYPAL_CHECKOUT")> class="payment_bg_color"</#if>>
								...
								<#if payment.responseData?has_content>
								<br />Gateway Response:
								<#list payment.responseData as response>
									<br />
									Transaction ID: <span>${response.referenceNum?if_exists}</span><br />
									Auth Code: <span>${response.gatewayCode?if_exists}</span><br />
									<#if response.gatewayMessage?has_content>Status String: <span>${response.gatewayMessage}</span><br /></#if>
									<#if response.gatewayFlag?has_content>Status Code: <span>${response.gatewayFlag}</span><br /></#if>
									<#if response.transactionDate?has_content>Date: <span>${response.transactionDate?string.short}</span><br /></#if>
								</#list>
								</#if>
							</div>
						</#list>
					...
					<#if billingAddress?has_content>
						...
						<div class="jqs-editable-info jqs-billing-phone">
							<form>
								...
								<input type="hidden" name="contactMechId" value="${billingTelecom.contactMechId?if_exists}" />
								<input name="orderId" type="hidden" value="${order.orderId}">
							</form>
						</div>
					<#else>
						...
						<div class="order-actions">
										...
										<div class="widget-body row proof-files">
											<div class="col-xs-12">
												<#if proofPDF?exists && proofPDF?has_content>
													<div><i class="fa fa-file pdf"> <a href="/admin/control/serveFileForStream?filePath=${proofPDF.contentPath}&amp;fileName=${proofPDF.contentName?default("_NA_")?replace("[^A-Za-z0-9\\.\\-\\_]","","r")}&amp;downLoad=Y">${proofPDF.contentName?default("_NA_")}</a></i></div>
												<#else>
													<div>No Proof Uploaded</div>
												</#if>
												<#if printFrontPDF?exists && printFrontPDF?has_content>
													<div><i class="fa fa-file pdf"> <a href="/admin/control/serveFileForStream?filePath=${printFrontPDF.contentPath}&amp;fileName=${printFrontPDF.contentName?default("_NA_")?replace("[^A-Za-z0-9\\.\\-\\_]","","r")}&amp;downLoad=Y"><!--${printFrontPDF.contentName?default("_NA_")}-->Download Front Print File</a></i></div>
												</#if>
												<#if printBackPDF?exists && printBackPDF?has_content>
													<div><i class="fa fa-file pdf"> <a href="/admin/control/serveFileForStream?filePath=${printBackPDF.contentPath}&amp;fileName=${printBackPDF.contentName?default("_NA_")?replace("[^A-Za-z0-9\\.\\-\\_]","","r")}&amp;downLoad=Y"><!--${printBackPDF.contentName?default("_NA_")}-->Download Back Print File</a></i></div>
												</#if>
											</div>
										</div>
									</div>
											...
											<div style="display: table; width: 100%;">
												...
												<div style="display: table-cell; width: 150px; vertical-align: top;">
													...
													<div class="jqs-proofParent" data-designId="${item.artwork.scene7DesignId?if_exists}" data-designParentId="${item.scene7DesignParentId?if_exists}">
														...
														<button class="btn btn-primary jqs-variabledatagrid jqs-designnow m-t-10">Edit Addressing</button>
														</#if>
							...
							<div class="row customizing-options">
								<div class="customCol-7">
									<#if item.attribute.addresses?has_content && item.artwork.scene7DesignId?has_content>
										<#assign addressBookId = Static["com.envelopes.addressbook.AddressBookEvents"].getAddressBookId(delegator, item.artwork.scene7DesignId, "0")?if_exists />
									</#if>
										...
										<div class="widget-body">
											<div class="customerComments">
												<#if orderItem.comments?has_content>
													<p><span>${orderItem.createdStamp?string.short} - </span>${orderItem.comments?if_exists}</p>
												</#if>
												<#if item.comments?has_content>
													<#list item.comments as comment>
														<p><span>${comment.createdStamp?string.short} - </span>${comment.message?if_exists}</p>
													</#list>
												</#if>
											</div>
										</div>
...
<script src="/html/js/admin/viewOrderLegacy.js?ts=13"></script>
<script type="text/javascript" src="<@ofbizContentUrl>/html/js/addons/jquery.minicolors.min.js</@ofbizContentUrl>"></script>
<script type="text/javascript" src="<@ofbizContentUrl>/html/texel/jquery.env.queue.js</@ofbizContentUrl>?ts=2"></script>
<script type="text/javascript" src="<@ofbizContentUrl>/html/texel/jquery.texel.widget.js</@ofbizContentUrl>?ts=3"></script>
<script type="text/javascript" src="<@ofbizContentUrl>/html/lib/envelopes-widget-util.js</@ofbizContentUrl>?ts=3"></script>
<script type="text/javascript" src="<@ofbizContentUrl>/html/addressing/addressingLegacy.js</@ofbizContentUrl>?ts=2"></script>
