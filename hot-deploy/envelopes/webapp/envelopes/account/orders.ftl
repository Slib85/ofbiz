...
<link href="<@ofbizContentUrl>/html/css/account/orders-redesign.css</@ofbizContentUrl>" rel="stylesheet" />
	...
	<div class="container orders">
		<#if (orders?size == 0)>
			<div data-alert class="alert-box warning radius">
				No <#if orderType == 'open'>Open <#elseif orderType == 'all'>All <#else></#if>orders available.
		...
		<div class="row reorder-button-container">
			<div class="button-regular reorder-button button-cta element-inactive jqs-reorder round-btn">Reorder</div>
		</div>
		<table class="table" id="orderList">
			...
			<tbody>
				<#list orders as order>
					<#if order.webSiteId == "ae" || order.webSiteId == "envelopes">
						<tr>
					<td class="order-info order-full-info">
						<span>
							<a href="<@ofbizUrl>/order-detail</@ofbizUrl>?orderId=${order.orderId}">${order.orderId}</a>
						</span>
							...
							<div>
								...
								<div class="order-status order-separate-info">
									<#if order.orderStatus == 'Cancelled' || order.orderStatus == 'Shipped'>
										${order.orderStatus}
									<#else>
                                    	<#if item.itemStatus == 'Created' || item.itemStatus == 'Backordered' || item.itemStatus == 'Outsourced' || item.itemStatus == 'Sent Purchase Order' || item.itemStatus == 'Syracuse' || item.itemStatus == 'Good to Proof' || item.itemStatus == 'Production Check' || item.itemStatus == 'Internal Proof Review' || item.itemStatus == 'Artwork Printed'>Processing<#elseif item.itemStatus == "Proof Ready for Review"><a href="<@ofbizUrl>/proofApproval</@ofbizUrl>">${item.itemStatus}</a><#else>${item.itemStatus}</#if>
									</#if>
								</div>
								<div class="actions">
									<a href="<@ofbizUrl>/order-detail</@ofbizUrl>?orderId=${order.orderId}">View Order Detail</a>
