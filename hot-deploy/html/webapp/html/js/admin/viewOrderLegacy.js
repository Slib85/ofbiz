...
function designNowEvent(element, loadAddressingFunction) {
	var index = element.closest('.jqs-proofParent').index('.jqs-proofParent');
	element.closest('.jqs-proofParent').find('.jqs-saveDesignChanges').removeClass('hidden');

	if (typeof texelProject[index] == 'undefined') {
		texelProject[index] = new TexelProject(index);
	}

	texelProject[index].loadTexel(loadAddressingFunction);
}
...
$(function() {
	...
	$('.jqs-variabledatagrid').on('click', function() {
		var index = $(this).closest('.jqs-proofParent').index('.jqs-proofParent');
		$(this).closest('.jqs-proofParent').find('.jqs-saveDesignChanges').removeClass('hidden');
...
function TexelProject(index) {
	...
	this.loadTexel = function(loadAddressingFunction) {
		if (typeof texelDiv != 'undefined' && $(texelDiv).hasClass('hidden')) {
			texelDiv.texel('onLoad');
		}
		if (typeof loadAddressingFunction == 'function') {
			loadAddressingFunction();
		}
	}
	...
	this.saveProject = function() {
		$.ajax({
			...
			data: {
				...
				'orderId': $('.jqs-orderId').html(),
				'orderItemSeqId': $('.jqs-proofParent:eq(' + projectIndex + ')').closest('.jqs-orderItem').attr('data-seqid'),
			...
			if(!data.success) {
				alert('An Error has occured while trying to save design.  Please contact King Khan.');
			}
			else {
				alert('Design Saved!');
			}
								...
								$.each(inputs, function (id, input){
									if (input.type == "text"){
										...
										if ($(input).attr('name') == 'addresses' && parseInt($(input).attr('value')) > 0) {
											designNowEvent($(input).closest('.jqs-orderItem').find('.jqs-designnow'), function() {
												var grid = $('#texel_0').texel('createAddressingGrid', [[]], 'manual', '2031759', '0', false);
												grid.VariableDataGrid('showGrid', 'manual');
											});
										}
									} else if (input.type == "checkbox"){
