...
function quoteRequestSubmit() {
    ...
    var formData = {
            additonalInfo: $('textarea[name="quoteDescribeProject"]').val(),
    ...
    $.ajax({
        ...
        if (data.success) {
            GoogleAnalytics.trackEvent('Quick Quuick Quote Request', 'Finish', data.quoteId);
            ...
            GoogleAnalytics.trackEvent('Quick Quote Request', 'Finish', 'Error');
        }
    ...
    $('[name="quoteRequestForm"] select,[name="quoteRequestForm"] textarea').on('change', function(e) {
        GoogleAnalytics.trackEvent('Quick Quote Request', 'CUSTOM', 'Start');
    });
}
