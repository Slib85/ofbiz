...
$('.jqs-approveOrRejectButton').on('click', function() {
	var parentElement = $(this).parentsUntil('.jqs-proofParent').parent();

	if ($(this).attr('data-do') == 'approve') {
        parentElement.find('.jqs-approveProof').removeClass('hidden');
	}
	else {
        parentElement.find('.jqs-rejectProof').removeClass('hidden');
	}
...
$('.jqs-goBack').on('click', function() {
    var parentElement = $(this).parentsUntil('.jqs-proofParent').parent();
    parentElement.find('.jqs-approveOrRejectButton').parent().removeClass('hidden');
	parentElement.find('.jqs-approveProof').removeClass('hidden').addClass('hidden');
	parentElement.find('.jqs-rejectProof').removeClass('hidden').addClass('hidden');
});
	...
	this.saveProject = function(element) {
		...
		$.ajax({
			...
			data: {
				...
				// WE REMOVED SETTING STATUS ID TO ART READY FOR REVIEW BECAUSE PRE-PRESS WOULD NOT GET A NEW WORKER FILE SET UP WHEN PROOF WAS REJECTED.
				//'statusId': (typeof element !== 'undefined' && element.closest('.jqs-proofParent').find('.jqs-customerComments').val().replace(/\s/g, '') != '' ? '' : 'ART_READY_FOR_REVIEW')
			},
			async: false,
			dataType: 'json',
			cache: false
