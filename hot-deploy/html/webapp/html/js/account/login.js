...
(function($) {
    $.extend({
            ...
            if(success) {
                $.ajax({
                    ...
                    if (data.success) {
                        ...
                        if(typeof reloadOnLogin !== 'undefined' && reloadOnLogin) {
                            window.location.reload(false);
                        }

                        if (typeof loadReOrderHistory == 'function') {
                            loadReOrderHistory();
                        }
                    }
                });
        ...
        closeLoginLayer: function() {
            if(typeof websiteId !== 'undefined' && websiteId != 'folders') {
        ...
        updateCartOnLogin: function() {
            $.ajax({
                type: 'GET',
                url: '/' + websiteId + '/control/getCart?miniCart=true',
                dataType: 'json',
                cache: false
            }).done(function(data) {
                $().updateMiniCart(JSON.stringify(data));
            });
        },
