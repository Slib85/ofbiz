        ...
        $('#texelFileUploadForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                if(data.success) {
                    ...
                    self.uploadGridData(self.cleanVariableData(data.variableData), true);
...
//                    console.log('RowChanged::' + this._getActiveCell()[0]);
    ...
    cleanVariableData: function(data) {
        if (typeof data != "undefined") {
            ...
            for (var a = 0; a < data.length; a++) {
                zipCode = undefined;
                zipCodeIndex = undefined;
                state = undefined;

                for (var b = 0; b < data[a].length; b++) {
                    if (typeof data[a][b] != "undefined" && data[a][b] != null) {
                        if (typeof state == "undefined" || state == null) {
                            state = data[a][b].match(/^(?:\s+|)((?:connecticut|massachusetts|maine|new(?:\s+|)hampshire|new(?:\s+|)jersey|puerto(?:\s+|)rico|rhode(?:\s+|)island|vermont))(?:\s+|)$/i);
                        }

                        if (typeof zipCode == "undefined" || zipCode == null) {
                            zipCode = data[a][b].match(/^(?:\s+|)([0-9]+)((?:\-[0-9]+(?:\s+|)$|(?:\s+|)$))/);
                        ...
                        if (typeof state != "undefined" && typeof zipCode != "undefined" && state != null && zipCode != null) {
                            var zipCodeLength = zipCode[1].length;

                            for (var x = 0; x < 5 - zipCodeLength; x++) {
                                zipCode[1] = "0" + zipCode[1];
                            }

                            data[a][zipCodeIndex] = zipCode[1] + (typeof zipCode[2] != "undefined" ? zipCode[2] : "");
                            break;
                        }
                    }
                }
