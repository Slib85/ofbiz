The B!GNAME Commerce Code Base
========

Welcome to the primary code base for the OfBiz project for B!GNAME commerce. This top section will help you get the project up and running.
For this tutorial we assume basic knoweldge of web development and Java. You are free to use which ever development environment you choose, but we suggest IntelliJ IDEA.

1. Obtain development database connection info from our IT team. (or a copy if you feel ambitious)
2. Edit the localmysql tag connection info in EntityEngine.xml to use the development database connection information.
3. Open your project settings in IntelliJ and set your compiler to use Java 8.
4. Create a new run configuration for gradle. The gradle project should be the same as the root directory of your project. For example C:\dev\ofbiz2017-dev2 on Windows.
5. In the run configuration add "ofbiz" as a run command.
6. Download nginx
7. edit the nginx configuration file location to point to the ofbiz project. For example C:/dev/ofbiz2017-dev2/hot-deploy/html/webapp/ where dev holds the nginx root folder.
8. Run nginx.
9. Run the application from the run configuration you created.
10. Go to localhost in your browser to ensure the application is running.
