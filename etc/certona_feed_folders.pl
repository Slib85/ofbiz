...
use constant MAPPING_FILE => "/usr/local/ofbiz/etc/folders-proxy_rewrite.txt";
...
my $variants_handle = $dbh->prepare(qq{
	SELECT
	    ...
	    case when pp.price is not null then pp.price else 0 end AS price,
	    case when pp.original_price is not null then pp.original_price else 0 end as original_price,
	    pc.primary_parent_category_id AS parent_category_id,
	    case when pp.quantity is not null then pp.quantity else 0 end AS quantity,
	    case when pp.colors is not null then pp.colors else 0 end AS colors,
        rc.rank AS rank,
	...
	WHERE
	    p.product_id = ?
	        AND (pp.product_price_type_id = 'DEFAULT_PRICE' or pp.price IS NULL)
...
my $output = IO::File->new(">/tmp/certona_folders.xml");
...
while (my $uniqueProd = $variants_unique_handle->fetchrow_hashref()) {
	...
	while (my $product = $variants_handle->fetchrow_hashref()) {
		...
		my $url_domain = "https://www.folders.com";
		my $url_path   = sprintf("/folders/control/product/~category_id=%s/~product_id=%s", $$product{"category_id"}, $$product{"product_id"});
		...
		} elsif($parentProductId ne "" && $parentProductId ne $$uniqueProd{"parent_product_id"}) {
			$doc->endTag("item");
			$doc->startTag("item", "name" => "id", "value" => $$uniqueProd{"parent_product_id"});
			$doc->dataElement("attribute",        ucfirst lc $$product{"product_type_id"}, "name" => "type");
			$doc->cdataElement("attribute",       $features{"size"}, "name" => "size");
...
foreach my $k (keys %category_data) {
	# generate the url parameters
	my $url_domain = "https://www.folders.com";
	my $url_path   = sprintf("/folders/control/category/~category_id=%s", $k);
	my $url_seo_path = $url_mapping{$url_path} || $url_path;
