...
var getOrderRecord = function(orderId) {
    var order = nlapiSearchRecord('salesorder', null, new nlobjSearchFilter('tranid', null, 'is', orderId));

    if(order != null && order.length > 1) {
        for(i in order) {
            var choice = getRecord(order[i].getRecordType(), order[i].getId());
            if(choice.getFieldValue('tranid') == orderId) {
...
var createOrder = function(data){
    ...
    if (typeof data.quoteId != 'undefined' && data.quoteId != null) {
        ...
        order = nlapiCreateRecord('salesorder');
    }

    // Set order Transaction ID (unique)
    if (typeof data.tranid != 'undefined'){
    ...
    customerId = addCustomerAndType(data.customer, 'customer', null, customerId).id;
    ...
    order.setFieldValue('customform', data.customform);
    ...
    if(typeof data.customer.shipping.companyName != 'undefined' && data.customer.shipping.companyName != null && data.customer.shipping.companyName != "") {
        ...
        order.setFieldValue('shipaddressee', "");
    }
    ...
    order.setFieldValue('shipphone', data.phone);
    order.setFieldValue('phone', data.phone);
    ...
    if(typeof data.customer.billing !== 'undefined') {
        ...
        order.setFieldValue('billphone', data.phone);
    }
    ...
    if(discountItem == null && data.discountcode !=null ) {
        discountItem = findItem({"sku":data.discountcode}).id;
    }
    order.setFieldValue('discountitem', discountItem);

    //insert record and get order id
    var orderId = nlapiSubmitRecord(order);
    var tranId = null;
    var externalId = null;
    ...
    if (data.terms != null || (typeof data.order_prefix != 'undefined' && data.order_prefix != '')) {
        ...
        if(data.terms == 30 && !ignoreDeposit) {
            createDeposit(data);
        }
    }
...
var updatePaymentData = function(order, data) {
    ...
    if (data.payment != null){
        ...
        order.setFieldValue('paymentmethod', data.payment.paymentmethod);
        ...
        order.setFieldValue('custbody_braintree_charged', bToS(true));
    }
...
var updateOrder = function(data) {
    ...
    if(orderId == null) {
        nlapiLogExecution('ERROR', 'Missing order', data.tranid);
    }
...
var createEstimate = function(data) {
    ...
    nlapiLogExecution('ERROR', 'createEstimate', JSON.stringify(data.tranid));
    ...
    if (estimate == null) {
        ...
        estimate.setFieldValue('customform', data.customform);

        estimate.setFieldValue('entity', customerId);
        estimate.setFieldValue('entitystatus', '7')
        ...
        estimate.setFieldValue('taxitem', '80778'); //data.taxitem
        estimate.setFieldValue('shipmethod', data.shipmethod);
        ...
        if(typeof data.customer.shipping.companyName != 'undefined' && data.customer.shipping.companyName != null && data.customer.shipping.companyName != "") {
            ...
            estimate.setFieldValue('shipaddressee', data.customer.shipping.companyName);
        } else {
            estimate.setFieldValue('shipcompany', "");
            estimate.setFieldValue('companyname', "");
            estimate.setFieldValue('shipaddressee', "");
        }
        ...
        estimate.setFieldValue('shipphone', data.phone);
        estimate.setFieldValue('phone', data.phone);
        ...
        if(typeof data.customer.billing !== 'undefined') {
            ...
            estimate.setFieldValue('billphone', data.phone);
        }
...
var findCustomer = function(data, type) {
    var customer = nlapiSearchRecord(type, null, new nlobjSearchFilter('externalid', null, 'is', data.partyId));
    if (customer == null) {
        return {"id": null};
    } else {
...
var addCustomerAndType = function(data, type, customer, id) {
    ...
    if(newRecord) {
        if(typeof data.lastname != 'undefined' && data.lastname != null && data.lastname != '') {
            customer.setFieldValue('lastname', data.lastname);
            customer.setFieldValue('firstname', data.firstname);
            customer.setFieldValue('entityid', data.firstname + ' ' + data.lastname + ' (' + data.partyId + ')' )
        }
        ...
        if(typeof data.resellerId != 'undefined' && data.resellerId != null && data.resellerId != '') {
            customer.setFieldValue('resalenumber', data.resellerId);
        }
        ...
        if(typeof data.phone != 'undefined' && data.phone != null && data.phone != '') {
            customer.setFieldValue('phone', data.phone);
        }
    ...
    customer.setFieldValue('taxitem', '80778');
    if((typeof data.isTrade != 'undefined' && data.isTrade != null && data.isTrade != '' && data.isTrade) || (typeof data.isNonProfit != 'undefined' && data.isNonProfit != null && data.isNonProfit != '' && data.isNonProfit)) {
        if(typeof data.isTrade != 'undefined' && data.isTrade != null && data.isTrade != '' && data.isTrade) {
            ...
            customer.setFieldValue('category', '14');
        }
    } else {
    ...
    if(typeof data.isNonTaxable != 'undefined' && data.isNonTaxable != null && data.isNonTaxable != '' && data.isNonTaxable) {
        ...
        customer.setFieldValue('custentity_ava_exemptcertno', 'Exempt');
    } else {
        customer.setFieldValue('taxable', 'T');
        customer.setFieldValue('custentity_ava_exemptcertno', null);
    }
...
var getInventory = function(data){
    var loadSearch = nlapiLoadSearch('item','customsearch211');
    ...
    do {
        ...
        for (var i in chunk) {
            response.push({
                "itemid":chunk[i].getValue('itemid'),
                "qty":chunk[i].getValue('quantityavailable'),
                "locationqty":chunk[i].getValue('locationquantityavailable'),
                "desc":chunk[i].getValue('displayname'),
                "dropship":chunk[i].getValue('isdropshipitem'),
                "dropshipOnly":chunk[i].getValue('custitem_drop_ship_only'),
                "reorderMultiple":chunk[i].getValue('reordermultiple'),
                "location":chunk[i].getValue('custitem_location')
            });
            resultIndex++;
        }
...
var getItemAverageCost = function(data){
    var loadSearch = nlapiLoadSearch('item', 'customsearch835');
    var searchResults = loadSearch.runSearch();
    var resultIndex = 0;
    var response = [];
    do {
...
var getItemWorkOrder = function(data){
    var loadSearch = nlapiLoadSearch('workorder', 'customsearch132');
    var searchResults = loadSearch.runSearch();
    var resultIndex = 0;
    var response = [];
    do {
...
var getAllFulfillments = function(data){
    var loadSearch = nlapiLoadSearch('transaction', 'customsearch471');
    var searchResults = loadSearch.runSearch();
    var resultIndex = 0;
    var response = [];
    do {
...
var getTracking = function(data){
    var map = {
        'amazon' : 'customsearch203',
        'quill' : 'customsearch364',
        'staples' : 'customsearch371',
        'jet' : 'customsearch914'
    };
...
var createDeposit = function(data) {
     ...
     */
    var despositData = { customform: '67', salesorder: order.getFieldValue('id'), entity: order.getFieldValue('entity') };
    deposit = nlapiCreateRecord('customerdeposit', despositData);
    ...
    if (data.payment != null) {
        ...
        deposit.setFieldValue('paymentmethod', data.payment.paymentmethod);
        ...
        deposit.setFieldValue('ccapproved', bToS(data.payment.ccapproved));
        deposit.setFieldValue('chargeit', bToS(data.payment.chargeit));
    }
...
var createPO = function(data) {
    ...
    //Create the PO data object
    var poData = { customform: '118', soid: order.getFieldValue('id'), shipgroup: '1', dropship: 'T', custid: order.getFieldValue('entity'), entity: data.vendorId };
...
function afterInvoiceSubmit(recordType, recordId) {
    ...
    if(invoiceRec.getFieldValue('entity') == '1006276' || invoiceRec.getFieldValue('entity') == '10912818') {
...
function afterSalesOrderSubmit(order, recordType, recordId) {
    ...
    if(order.getFieldValue('entity') == '1006276' || order.getFieldValue('entity') == '1038240') {
        ...
        for(var i = 1; i < numItems*2; i++) {
            if(i == (numItems*2)-1) {
                ...
                order.setCurrentLineItemValue('item', 'item', (order.getFieldValue('entity') == '1006276') ? '18359' : '18444');
        ...
        nlapiSubmitRecord(order);
    } else if(order.getFieldValue('entity') == '10912818') {
        var numItems = order.getLineItemCount('item');
        ...
        for(var i = 1; i < numItems*2; i++) {
            if(i == (numItems*2)-1) {
                ...
                order.selectNewLineItem('item')
                order.setCurrentLineItemValue('item', 'item', '19413');
                order.setCurrentLineItemValue('item', 'quantity', 1);
        ...
        nlapiSubmitRecord(order);
    } else if(order.getFieldValue('entity') == '16989577') {
        var numItems = order.getLineItemCount('item');
        ...
        for(var i = 1; i < numItems*2; i++) {
            if(i == (numItems*2)-1) {
                ...
                order.selectNewLineItem('item')
                order.setCurrentLineItemValue('item', 'item', '23162');
                order.setCurrentLineItemValue('item', 'quantity', 1);
    ...
    if(typeof data.orderId !== 'undefined' && data.orderId != null) {
        ...
        if(typeof order != 'undefined' && order != null) {
            ...
            for (var k = 0; k < itemFulfillments.length; k++) {
                ...
                try {
                    itemFulfillment.setLineItemValue('packageups', 'packagetrackingnumberups', 1, data.trackingNumber);
                    itemFulfillment.setLineItemValue('packageups', 'packageweightups', 1, data.packageWeight);
                } catch (err) {
...
var getOrderItemPO = function(data) {
    ...
    var items = [];
    for (var i = 1; i <= order.getLineItemCount('item'); i++) {
        if (typeof data.lineItem != 'undefined' && order.getLineItemValue('item', 'custcol_line_item_sequence', i) == data.lineItem && typeof order.getLineItemValue('item', 'poid', i) != 'undefined' && order.getLineItemValue('item', 'poid', i) != null) {
            po = findPO({'id': order.getLineItemValue('item', 'poid', i)});
        } else if (typeof data.lineItem != 'undefined' && order.getLineItemValue('item', 'custcol_line_item_sequence', i) == data.lineItem && typeof order.getLineItemValue('item', 'custcolreference_po_id', i) != 'undefined' && order.getLineItemValue('item', 'custcolreference_po_id', i) != null) {
