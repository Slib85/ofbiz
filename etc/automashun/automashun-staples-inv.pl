...
if (!$httpResponse->is_error){
	...
	foreach my $key ( @perl_array ){
		...
		$quantities{$key->{'itemid'}} = (defined $key->{'dropship'} && $key->{'dropship'} eq "T") ? '1000' : $key->{'locationqty'};
...
while (my $inventory_override_row = $inventory_override_data->fetchrow_hashref()) {
	my $quantity = $$inventory_override_row{"staples"};
	my $sku = $$inventory_override_row{"product_id"};
