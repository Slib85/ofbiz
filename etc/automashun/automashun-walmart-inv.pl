...
my $ofbizDirectory = "/usr/local/ofbiz";
#my $ofbizDirectory = "c:/dev/ofbiz2017";

my $ofbiz = Ofbiz->new(conf => "$ofbizDirectory/framework/entity/config/entityengine.xml", datasource => "localmysql");
...
my $inputFile = "$ofbizDirectory/etc/automashun/staples-inv-sample.xls";
my $outputFile = '/tmp/walmart-inv.xls';
...
open (SI, '>', '/tmp/EvelopesCom-Walmartcom-Inventory.csv');
print SI "SKU,Quantity,Status,Discontinue Date" . "\n";
open (SIB, '>', '/tmp/EvelopesCom-Walmartcom-Inventory_b.csv');
...
my $walmartSkusFile = '/usr/local/ofbiz/etc/automashun/walmart-skus.csv';
open(my $walmartSkusFH, '<:encoding(UTF-8)', $walmartSkusFile) or die "Could not open file '$walmartSkusFile': $!";
my $output = "";
my $counter = 0;
while (my $row = <$walmartSkusFH>) {
    if ($counter != 0) {
        #================================================
        # Walmart Row Data Index Definitions
        # 0: ID
        # 1: MODEL #
        # 2: WalmartSKU
        # 3: ENVELOPES.com SKU,
        ...
        my @walmartRowData = split(",", $row);
        $walmartRowData[1] =~ s/^\s+|\s+$//g;
        $skuLookup{$walmartRowData[1]} = {
            "sku" => $walmartRowData[3],
            "qty" => $walmartRowData[4],
            "wamlart_sku" => $walmartRowData[2]
        };
    }
...
foreach my $key ( keys %skuLookup ){
	...
	$sampleRow[16] = $skuLookup{$key}->{walmart_sku};
		...
		if($qty ne "" && defined $breaks{$skuLookup{$key}->{sku}} && ($skuLookup{$key}->{qty} ~~ $breaks{$skuLookup{$key}->{sku}})) {
			...
			foreach my $break (@breakArray) {
				if ($skuLookup{$key}->{qty} ne "" && $skuLookup{$key}->{qty} eq $break) {
					...
					print SI $key . "," . (int($qty) > 0 ? int($qty / $skuLookup{$key}->{qty}) : 0) . "," . (int($qty) < 0 ? "Discontinued" : "") . "," . $curDate->ymd('') . "\n";
				}
			}
			...
			$sampleRow[13] = "";
			print SI $key . "," . int($qty / $skuLookup{$key}->{qty}) . ",," . "\n";
			if(defined $locations{$skuLookup{$key}->{sku}} && $locations{$skuLookup{$key}->{sku}} eq "2") {
				print SIB $key . "," . int($qty / $skuLookup{$key}->{qty}) . ",," . "\n";
			}
		} else {
			$sampleRow[2] = "NO";
			$sampleRow[3] = 0;
...
sub sendEmail{
	...
	for (@receipients){
		$mail->send(
					...
					-verbose=>'1',
					-body=> '',
					-attachments=>'/tmp/EvelopesCom-Walmartcom-Inventory.csv'
					);
	}
	$mail->bye;
}
