...
my $inventory_override_data = $dbh->prepare(qq{
	...
	WHERE 
		office_depot IS NOT NULL
});

$inventory_override_data->execute();

while (my $inventory_override_row = $inventory_override_data->fetchrow_hashref()) {
	$quantities{$$inventory_override_row{"product_id"}} = $$inventory_override_row{"office_depot"}; 
}
	...
	if($CSV->parse($_)) {
		...
		if(!defined $quantities{$sku}) {
			...
			if($netsuiteQty ne "" && int($netsuiteQty / $qty) > 0) {
				my $row = [ $columns[0], $columns[1], int($netsuiteQty / $qty) ];
				$outputCSV->print(\*ODCSV, $row);
			} else {
				my $row = [ $columns[0], $columns[1], '0' ];
				$outputCSV->print(\*ODCSV, $row);
