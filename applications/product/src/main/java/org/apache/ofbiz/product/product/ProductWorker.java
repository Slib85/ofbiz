...
public final class ProductWorker {
    ...
    public static GenericValue getParentProduct(String productId, Delegator delegator) {
        ...
        try {
            GenericValue product = EntityQuery.use(delegator).from("Product").where("productId", productId).cache().queryOne();
            if(product != null && UtilValidate.isNotEmpty(product.getString("parentProductId"))) {
                _parentProduct = EntityQuery.use(delegator).from("Product").where("productId", product.getString("parentProductId")).cache().queryOne();
            }
            /*
    ...
    //get product stock
    public static GenericValue getProductStock(String productId, Delegator delegator) {
        GenericValue _productStock = null;
        if (productId == null) {
            Debug.logWarning("Bad product id", module);
        }

        try {
            GenericValue productStock = EntityQuery.use(delegator).from("ProductStock").where("productId", productId).cache().queryOne();
            if(productStock != null) {
                _productStock = productStock;
            }
        } catch (GenericEntityException e) {
            throw new RuntimeException("Entity Engine error getting Product Stock (" + e.getMessage() + ")");
        }
