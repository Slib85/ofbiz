...
public class OrderReadHelper {
    ...
    public List<GenericValue> getPaymentPreferences() {
        if (paymentPrefs == null) {
            try {
                paymentPrefs = orderHeader.getRelated("OrderPaymentPreference", null, UtilMisc.toList("createdStamp DESC"), false);
            } catch (GenericEntityException e) {
                Debug.logError(e, module);
            }
        }
    ...
    public GenericValue getShipmentMethodType(String shipGroupSeqId) {
        GenericValue shipmentMethodType = null;
        try {
            GenericValue shipGroup = EntityQuery.use(orderHeader.getDelegator()).from("OrderItemShipGroup").where("orderId", orderHeader.getString("orderId"), "shipGroupSeqId", shipGroupSeqId).queryOne();
            if (shipGroup != null && UtilValidate.isNotEmpty(shipGroup.getString("shipmentMethodTypeId"))) {
                shipmentMethodType = EntityQuery.use(orderHeader.getDelegator()).from("ShipmentMethodType").where("shipmentMethodTypeId", shipGroup.getString("shipmentMethodTypeId")).queryOne();
            }
        } catch (GenericEntityException e) {
    ...
    public String getGenericShippingMethod(String shipGroupSeqId) {
        return getShippingMethod(shipGroupSeqId, true);
    }

    public String getShippingMethod(String shipGroupSeqId) {
        return getShippingMethod(shipGroupSeqId, false);
    }

    public String getShippingMethod(String shipGroupSeqId, boolean showGeneric) {
        try {
            GenericValue shipGroup = EntityQuery.use(orderHeader.getDelegator()).from("OrderItemShipGroup").where("orderId", orderHeader.getString("orderId"), "shipGroupSeqId", shipGroupSeqId).queryOne();
            ...
            if (shipGroup != null) {
                ...
                if (carrierShipmentMethod != null) {
                    ...
                    if (shipmentMethodType != null) {
                        return UtilFormatOut.checkNull(showGeneric ? UtilFormatOut.checkNull(shipmentMethodType.getString("genericDescription")) : UtilFormatOut.checkNull(shipGroup.getString("carrierPartyId")) + " " + UtilFormatOut.checkNull(shipmentMethodType.getString("description")));
                    }
                }
    ...
    public String getShippingMethodType(String shipGroupSeqId) {
        try {
    ...
    public String getShippingMethodDesc(String shipGroupSeqId) {
        try {
            ...
            if (shipGroup != null) {
                ...
                if (carrierShipmentMethod != null) {
                    return carrierShipmentMethod.getRelatedOne("ShipmentMethodType", false).getString("description");
                }
            }
        } catch (GenericEntityException e) {
            Debug.logWarning(e, module);
