...
public final class ProductPromoWorker {
        ...
        try {
            ...
            if (productStore != null) {
                ...
                while (productStorePromoAppls != null && productStorePromoAppls.hasNext()) {
                    ...
                    //if a string of website ids are passed, then the promo will only work on those ids, if none is passed promo is allowed on all ids
                    String webSiteId = cart.getWebSiteId();
                    if(UtilValidate.isNotEmpty(webSiteId) && UtilValidate.isNotEmpty(productStorePromoAppl.getString("webSiteId")) && !productStorePromoAppl.getString("webSiteId").contains(webSiteId)) {
                        if (Debug.verboseOn()) Debug.logVerbose("Skipping promotion with id [" + productStorePromoAppl.getString("productPromoId") + "] because it is not allowed for " + webSiteId + ".", module);
                        continue;
                    }
                        ...
                        while (condResult && promoRulesItr != null && promoRulesItr.hasNext()) {
                            ...
                            while (condResult && productPromoConds != null && productPromoConds.hasNext()) {
                                ...
                                if ("PPIP_PARTY_ID".equals(productPromoCond.getString("inputParamEnumId"))) {
                                    ...
                                    condResult = checkCondition(productPromoCond, cart, delegator, dispatcher, nowTimestamp);
                                } else if ("PPIP_ORST_HIST".equals(productPromoCond.getString("inputParamEnumId"))) {
                                    condResult = checkCondition(productPromoCond, cart, delegator, dispatcher, nowTimestamp);
                                }
                            }
                        }
    ...
    public static List<GenericValue> getProductStorePromotions(ShoppingCart cart, Timestamp nowTimestamp, LocalDispatcher dispatcher) {
        ...
        try {
            ...
            while (prodCatalogPromoAppls != null && prodCatalogPromoAppls.hasNext()) {
                ...
                //if a string of website ids are passed, then the promo will only work on those ids, if none is passed promo is allowed on all ids
                String webSiteId = cart.getWebSiteId();
                if(UtilValidate.isNotEmpty(webSiteId) && UtilValidate.isNotEmpty(prodCatalogPromoAppl.getString("webSiteId")) && !prodCatalogPromoAppl.getString("webSiteId").contains(webSiteId)) {
                    if (Debug.verboseOn()) Debug.logVerbose("Skipping promotion with id [" + prodCatalogPromoAppl.getString("productPromoId") + "] because it is not allowed for " + webSiteId + ".", module);
                    continue;
                }
    ...
    public static BigDecimal doPromotions(ShoppingCart cart, LocalDispatcher dispatcher) {
        return ProductPromoWorker.doPromotions(cart, null, dispatcher);
    }

    public static BigDecimal doPromotions(ShoppingCart cart, List<GenericValue> productPromoList, LocalDispatcher dispatcher) {
        ...
        return cart.getDisplayGrandTotal();
    }
                ...
                for (GenericValue productPromo : productPromoList) {
                    ...
                    if (UtilValidate.isNotEmpty(productPromoRules)) {
                        ...
                        if (requireCode) {
                            ...
                            if(partyId != null) {
                                GenericValue promoExclusion = EntityQuery.use(delegator).from("ProductPromoExclusion").where("productPromoId", productPromo.getString("productPromoId"), "partyId", partyId).cache().queryOne();
    ...
    public static Long getProductPromoCodeUseLimit(GenericValue productPromoCode, String partyId, Delegator delegator) throws GenericEntityException {
        ...
        if (codeUseLimitPerCustomer != null && codeUseLimitPerCustomer.longValue() > 0) {
            long productPromoCustomerUseSize = 0;
            if (UtilValidate.isNotEmpty(partyId)) {
                // check to see how many times this has been used for other orders for this customer, the remainder is the limit for this order
                EntityCondition checkCondition = EntityCondition.makeCondition(UtilMisc.toList(
        ...
        if (codeUseLimitPerCode != null && codeUseLimitPerCode.longValue() > 0) {
            // check to see how many times this has been used for other orders for this customer, the remainder is the limit for this order
            EntityCondition checkCondition = EntityCondition.makeCondition(UtilMisc.toList(
                    EntityCondition.makeCondition("productPromoCodeId", EntityOperator.EQUALS, productPromoCodeId),
                    EntityCondition.makeCondition("statusId", EntityOperator.NOT_EQUAL, "ORDER_REJECTED"),
    ...
    public static String checkCanUsePromoCode(String productPromoCodeId, String partyId, Delegator delegator, ShoppingCart cart, Locale locale) {
        try {
            GenericValue productPromoCode = EntityQuery.use(delegator).from("ProductPromoCode").where("productPromoCodeId", productPromoCodeId).cache().queryOne();
            ...
            GenericValue productPromo = EntityQuery.use(delegator).from("ProductPromo").where("productPromoId", productPromoCode.getString("productPromoId")).cache().queryOne();

            if (cart != null) {
                boolean tradeMember = cart.getTradeStatus() || cart.getNonProfitStatus();
                boolean canBeApplied = false;
                boolean isStackable = false;

                if(tradeMember) {
                    if("Y".equalsIgnoreCase(productPromo.getString("canTradeUse"))) {
                        canBeApplied = true;
                        if("Y".equalsIgnoreCase(productPromo.getString("isStackable"))) {
                            isStackable = true;
                        }
                    } else {
                        return "Sorry, this offer is not available for Trade customers.";
                    }
                } else {
                    canBeApplied = true;
                    if("Y".equalsIgnoreCase(productPromo.getString("isStackable"))) {
                ...
                if(!isStackable) {
                    cart.removeProductPromoCode(null, null);
                }

                if(!canBeApplied) {
                    return "Sorry, this is an invalid coupon code.";
                }

                //see promo exclusions
                if(partyId == null && cart.getPartyId() != null) {
            ...
            if(partyId != null) {
                GenericValue promoExclusion = EntityQuery.use(delegator).from("ProductPromoExclusion").where("productPromoId", productPromoCode.getString("productPromoId"), "partyId", partyId).cache().queryOne();
                if (promoExclusion != null) {
                    return "Sorry, this offer is not available for your account.";
                }
            }
...
//                        BigDecimal orderSubTotalAndCartSubTotal = orderSubTotal.add(cart.getSubTotal());
    ...
    public static void performAction(ActionResultInfo actionResultInfo, GenericValue productPromoAction, ShoppingCart cart, Delegator delegator, LocalDispatcher dispatcher, Timestamp nowTimestamp) throws GenericEntityException, CartItemModifyException {
        ...
        if ("PROMO_SERVICE".equals(productPromoActionEnumId)) {
            ...
            boolean addToCart = "Y".equalsIgnoreCase(productPromoAction.getString("addToCart")) ? true : false;

            //Set<String> productIds = ProductPromoWorker.getPromoRuleActionProductIds(productPromoAction, delegator, nowTimestamp);
            String productIds = productPromoAction.getString("productId");
            ...
            for (ShoppingCartItem cartItem : cart.items()) {
                ...
                if (!cartItem.getIsPromo()
                        ...
                        && quantityDesired.compareTo(cartItem.getQuantity()) == 0
                   ) {
                    discountAmountTotal = discountAmountTotal.add(cartItem.getItemSubTotal());
                }
                //each item can only be redeemed once, so remove it from string so it doesnt match again
                productIds.replace(cartItem.getProductId(), "");
                if(parentProductId != null) { productIds.replace(parentProductId, ""); }
            }

            //if we didnt find the item in the cart, add it
            if(addToCart && discountAmountTotal.compareTo(BigDecimal.ZERO) == 0) {
                //TODO
            }
            ...
            BigDecimal amount = discountAmountTotal.multiply(percentage);
            if (amount.compareTo(BigDecimal.ZERO) != 0 && amount.compareTo(subTotal) < 0) {
            ...
            //we will only loop through the item list if we don't allow clearance, if we do, no sense in stepping into this code
            boolean allowClearance = "Y".equalsIgnoreCase(productPromoAction.getString("allowClearance"));
            if(!allowClearance) {
                for (ShoppingCartItem cartItem : cart.items()) {
                    boolean isClearance = "Y".equalsIgnoreCase((cartItem.getProduct()).getString("onClearance")) || "Y".equalsIgnoreCase((cartItem.getProduct()).getString("onSale"));
                    if (isClearance) {
                        continue;
                    }
                    subTotal = subTotal.add(cartItem.getItemSubTotal());
                }
            } else {
                subTotal = cart.getSubTotalForPromotions();
            }

            BigDecimal percentage = (productPromoAction.get("amount") == null ? BigDecimal.ZERO : (productPromoAction.getBigDecimal("amount").movePointLeft(2))).negate();
            BigDecimal amount = subTotal.multiply(percentage);
            ...
            Map<String, String> shipTaxPostalAddress = cart.getShipTaxPostalAddress();

            if(UtilValidate.isNotEmpty(shipTaxPostalAddress)) {
                if(UtilValidate.isNotEmpty(shipTaxPostalAddress.get("stateProvinceGeoId"))) {
                    stateProvinceGeoId = shipTaxPostalAddress.get("stateProvinceGeoId");
                }
                if(UtilValidate.isNotEmpty(shipTaxPostalAddress.get("countryGeoId"))) {
                    countryGeoId = shipTaxPostalAddress.get("countryGeoId");
                }
            } else {
                GenericValue shippingAddress = cart.getShippingAddress();
                if(UtilValidate.isNotEmpty(shippingAddress)) {
                    if (UtilValidate.isNotEmpty(shippingAddress.getString("stateProvinceGeoId"))) {
                        stateProvinceGeoId = shippingAddress.getString("stateProvinceGeoId");
                    }
                    if (UtilValidate.isNotEmpty(shippingAddress.getString("countryGeoId"))) {
                        countryGeoId = shippingAddress.getString("countryGeoId");
                    }
                }
            }

            boolean qualifiesForFreeShip = true;
            String provinceExclusions = UtilProperties.getPropertyValue("envelopes", "promo.freeship.province.exclusions");
            String countryExclusions = UtilProperties.getPropertyValue("envelopes", "promo.freeship.country.exclusions");

            if(UtilValidate.isNotEmpty(stateProvinceGeoId)) {
                if(provinceExclusions != null && provinceExclusions.contains(stateProvinceGeoId)) {
                    qualifiesForFreeShip = false;
                }
            }
            if(UtilValidate.isNotEmpty(countryGeoId)) {
                if(countryExclusions != null && countryExclusions.contains(countryGeoId)) {
                    qualifiesForFreeShip = false;
                }
            }
            ...
            if(qualifiesForFreeShip) {
                ...
                String promoShipmentMethodTypes = productPromoAction.getString("productId"); //reusing existing field, may not make sense but best use for it
                String[] promoShipmentMethodTypeList = promoShipmentMethodTypes.split(",\\s*"); //split it because multiple options can be entered here
                List<String> shipList = Arrays.asList(promoShipmentMethodTypeList);
                for(String promoShipmentMethodTypeId : shipList) {
                    if (UtilValidate.isEmpty(promoShipmentMethodTypeId) || shipmentMethodTypeId.equals(promoShipmentMethodTypeId)) {
                        BigDecimal percentage = (productPromoAction.get("amount") == null ? BigDecimal.ZERO : (productPromoAction.getBigDecimal("amount").movePointLeft(2))).negate();
        ...
        } else if ("PROMO_SCENE7_COUPON_FLAT".equals(productPromoActionEnumId)) {
            // go through the cart items for printed scene7 products
            boolean allowClearance = "Y".equalsIgnoreCase(productPromoAction.getString("allowClearance"));
            String templateSeries = productPromoAction.getString("productId"); //reusing existing field, may not make sense but best use for it

            boolean hasScene7Item = false;
            for (ShoppingCartItem cartItem : cart.items()) {
                boolean isClearance = "Y".equalsIgnoreCase((cartItem.getProduct()).getString("onClearance")) || "Y".equalsIgnoreCase((cartItem.getProduct()).getString("onSale"));
                if(!allowClearance && isClearance) {
                    continue;
                }

                //if we do not specify template ids, we just want to see if its scene7 order
                if(UtilValidate.isEmpty(templateSeries)) {
                    Map<String, Object> envArtworkAttributes = (Map<String, Object>) cartItem.getAttribute("envArtworkAttributes");
                    for (String key : envArtworkAttributes.keySet()) {
                        if (key.equals("artworkSource") && "SCENE7_ART_ONLINE".equalsIgnoreCase((String) envArtworkAttributes.get(key))) {
                            hasScene7Item = true;
                            break;
                        }
                    }
                } else {
                    //check if the template used matches the template that we want to discount
                    Map<String, Object> envPriceCalcAttributes = (Map<String, Object>) cartItem.getAttribute("envPriceCalcAttributes");
                    String[] templateArray = templateSeries.split(",\\s*");
                    for (String templateId: templateArray) {
                        if(UtilValidate.isNotEmpty(envPriceCalcAttributes.get("templateId")) && UtilValidate.isNotEmpty(templateId) && templateId.equalsIgnoreCase((String) envPriceCalcAttributes.get("templateId"))) {
                            hasScene7Item = true;
                            break;
                        } else if(UtilValidate.isNotEmpty(envPriceCalcAttributes.get("templateId")) && UtilValidate.isNotEmpty(templateId) && ((String) envPriceCalcAttributes.get("templateId")).startsWith(templateId)) {
                            hasScene7Item = true;
                            break;
                        }
                    }
            ...
            BigDecimal amount = hasScene7Item ? (productPromoAction.get("amount") == null ? BigDecimal.ZERO : productPromoAction.getBigDecimal("amount")).negate() : BigDecimal.ZERO;
            // if amount is greater than the order sub total, set equal to order sub total, this normally wouldn't happen because there should be a condition that the order total be above a certain amount, but just in case...
            BigDecimal subTotal = cart.getSubTotalForPromotions();
            if (amount.negate().compareTo(subTotal) > 0) {
                amount = subTotal.negate();
        ...
        } else if ("PROMO_SCENE7_COUPON".equals(productPromoActionEnumId)) {
            // go through the cart items for printed scene7 products
            boolean allowClearance = "Y".equalsIgnoreCase(productPromoAction.getString("allowClearance"));
            String templateSeries = productPromoAction.getString("productId"); //reusing existing field, may not make sense but best use for it
            ...
            for (ShoppingCartItem cartItem : cart.items()) {
                ...
                if(UtilValidate.isEmpty(templateSeries)) {
                    ...
                    for (String key : envArtworkAttributes.keySet()) {
                        if (key.equals("artworkSource") && "SCENE7_ART_ONLINE".equalsIgnoreCase((String) envArtworkAttributes.get(key))) {
                            amount = amount.add(cartItem.getItemSubTotal());
                        }
                    }
                } else {
                    //check if the template used matches the template that we want to discount
        ...
        } else if ("PROMO_PRINTED_COUPON_FLAT".equals(productPromoActionEnumId)) {
            // go through the cart items for printed products
            boolean allowClearance = "Y".equalsIgnoreCase(productPromoAction.getString("allowClearance"));

            boolean hasPrintedItem = false;
            for (ShoppingCartItem cartItem : cart.items()) {
                boolean isClearance = "Y".equalsIgnoreCase((cartItem.getProduct()).getString("onClearance")) || "Y".equalsIgnoreCase((cartItem.getProduct()).getString("onSale"));
                if(!allowClearance && isClearance) {
                    continue;
                ...
                Map<String, Object> envArtworkAttributes = (Map<String, Object>) cartItem.getAttribute("envArtworkAttributes");
                for (String key : envArtworkAttributes.keySet()) {
                    if (key.equals("artworkSource") && UtilValidate.isNotEmpty(envArtworkAttributes.get(key))) {
                        hasPrintedItem = true;
                        break;
                    }
                }
            ...
            BigDecimal amount = hasPrintedItem ? (productPromoAction.get("amount") == null ? BigDecimal.ZERO : productPromoAction.getBigDecimal("amount")).negate() : BigDecimal.ZERO;
            // if amount is greater than the order sub total, set equal to order sub total, this normally wouldn't happen because there should be a condition that the order total be above a certain amount, but just in case...
            BigDecimal subTotal = cart.getSubTotalForPromotions();
            if (amount.negate().compareTo(subTotal) > 0) {
                amount = subTotal.negate();
        ...
        } else if ("PROMO_PRINTED_COUPON".equals(productPromoActionEnumId)) {
            // go through the cart items for printed products
            boolean allowClearance = "Y".equalsIgnoreCase(productPromoAction.getString("allowClearance"));
        ...
        } else if ("PROMO_PLAIN_COUPON".equals(productPromoActionEnumId)) {
            // go through the cart items for plain products
            boolean allowClearance = "Y".equalsIgnoreCase(productPromoAction.getString("allowClearance"));
            ...
            for (ShoppingCartItem cartItem : cart.items()) {
                ...
                Map<String, Object> envArtworkAttributes = (Map<String, Object>) cartItem.getAttribute("envArtworkAttributes");
                if (UtilValidate.isEmpty(envArtworkAttributes.get("artworkSource"))) {
                    amount = amount.add(cartItem.getItemSubTotal());
                }
            }
        ...
        } else if ("PROMO_ADDRESSING_COUPON_FLAT".equals(productPromoActionEnumId)) {
            // go through the cart items for printed products
            boolean allowClearance = "Y".equalsIgnoreCase(productPromoAction.getString("allowClearance"));

            boolean hasAddressingItem = false;
            for (ShoppingCartItem cartItem : cart.items()) {
                boolean isClearance = "Y".equalsIgnoreCase((cartItem.getProduct()).getString("onClearance")) || "Y".equalsIgnoreCase((cartItem.getProduct()).getString("onSale"));
                if(!allowClearance && isClearance) {
                    continue;
                ...
                Map<String, Object> envPriceCalcAttributes = (Map<String, Object>) cartItem.getAttribute("envPriceCalcAttributes");
                for (String key : envPriceCalcAttributes.keySet()) {
                    if (key.equals("addresses") && UtilValidate.isNotEmpty(envPriceCalcAttributes.get(key)) && envPriceCalcAttributes.get(key) instanceof Integer && ((Integer) envPriceCalcAttributes.get(key)) > 0) {
                        hasAddressingItem = true;
                        break;
                    }
                }
            }

            BigDecimal amount = hasAddressingItem ? (productPromoAction.get("amount") == null ? BigDecimal.ZERO : productPromoAction.getBigDecimal("amount")).negate() : BigDecimal.ZERO;
            // if amount is greater than the order sub total, set equal to order sub total, this normally wouldn't happen because there should be a condition that the order total be above a certain amount, but just in case...
            BigDecimal subTotal = cart.getSubTotalForPromotions();
            if (amount.negate().compareTo(subTotal) > 0) {
                amount = subTotal.negate();
        ...
        } else if ("PROMO_ADDRESSING_COUPON".equals(productPromoActionEnumId)) {
            // go through the cart items for printed products
            boolean allowClearance = "Y".equalsIgnoreCase(productPromoAction.getString("allowClearance"));
        ...
        } else if ("PROMO_FREE_SAMPLES".equals(productPromoActionEnumId)) {
            if (cart.getTradeStatus()) {
                BigDecimal numOfSamples = cart.numOfSamplesInCart();

                //Amount is discount amount per sample
                BigDecimal amount = BigDecimal.ZERO;
                if (numOfSamples.compareTo(new BigDecimal(freeSamples)) >= 0) {
                    amount = productPromoAction.getBigDecimal("quantity").multiply(productPromoAction.getBigDecimal("amount"));
                } else {
                    amount = numOfSamples.multiply(productPromoAction.getBigDecimal("amount"));
                }

                /**
                 * This is a temporary hack to prevent value from going into negative
                 */
                if ((cart.getGrandTotal().subtract(amount)).compareTo(BigDecimal.ZERO) < 0) {
                   amount = amount.add(cart.getGrandTotal().subtract(amount));
                }
    ...
    public static void doOrderItemPromoAction(GenericValue productPromoAction, ShoppingCartItem cartItem, BigDecimal amount, String amountField, Delegator delegator) {
        ...
        GenericValue orderAdjustment = delegator.makeValue("OrderAdjustment",
                UtilMisc.toMap("orderAdjustmentTypeId", "PROMOTION_ADJUSTMENT", amountField, amount,
                        "productPromoActionEnumId", productPromoAction.get("productPromoActionEnumId"),
    ...
    public static boolean isProductPromoActionOfType(Delegator delegator, String productPromoCodeId, String productPromoActionEnumId) throws GenericEntityException {
        if(UtilValidate.isNotEmpty(productPromoCodeId) && UtilValidate.isNotEmpty(productPromoActionEnumId)) {
            ...
            if(productPromoCode != null) {
                List<GenericValue> productPromoActions = delegator.findByAnd("ProductPromoAction", UtilMisc.toMap("productPromoId", productPromoCode.getString("productPromoId"), "productPromoActionEnumId", productPromoActionEnumId), null, true);
                return UtilValidate.isNotEmpty(productPromoActions);
            }
        }
    ...
    public static BigDecimal getShippingRequirement(Delegator delegator, String productPromoCodeId) throws GenericEntityException {
        if(UtilValidate.isNotEmpty(productPromoCodeId)) {
            GenericValue productPromoCode = delegator.findOne("ProductPromoCode", UtilMisc.toMap("productPromoCodeId", productPromoCodeId), true);
            if(productPromoCode != null) {
                GenericValue productPromoAction = EntityUtil.getFirst(delegator.findByAnd("ProductPromoAction", UtilMisc.toMap("productPromoId", productPromoCode.getString("productPromoId")), UtilMisc.toList("productPromoActionSeqId ASC"), true));
                return (UtilValidate.isEmpty(productPromoAction) || (UtilValidate.isNotEmpty(productPromoAction) && productPromoAction.get("amount") == null)) ? BigDecimal.ZERO : productPromoAction.getBigDecimal("amount");
            }
        }
