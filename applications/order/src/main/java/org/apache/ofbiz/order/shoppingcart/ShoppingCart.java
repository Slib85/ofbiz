...
public class ShoppingCart implements Iterable<ShoppingCartItem>, Serializable {
    ...
    private String orderEmailContactMechId = null;
    ...
    // Tag if the user is trade or not
    protected boolean isTrade = false;

    // Tag if the user is non profit or not
    protected boolean isNonProfit = false;

    // Tag if the user is non taxable or not
    protected boolean isNonTaxable = false;
    ...
    protected Map<String, String> shipTaxPostalAddress; //holds the postalCode for the shipping/tax calculation

    protected boolean ignoreProductPriceLookup = false;
    ...
    public ShoppingCart(ShoppingCart cart) {
        ...
        this.orderEmailContactMechId = cart.getOrderEmailContactMechId();
    ...
    public Map<String, String> getShipTaxPostalAddress() {
        return this.shipTaxPostalAddress;
    }

    public void setShipTaxPostalAddress(Map<String, String> shipTaxPostalAddress) {
        this.shipTaxPostalAddress = shipTaxPostalAddress;
    }

    public boolean getIgnoreProductPriceLookup() {
        return this.ignoreProductPriceLookup;
    }

    public void setIgnoreProductPriceLookup(boolean ignoreProductPriceLookup) {
        this.ignoreProductPriceLookup = ignoreProductPriceLookup;
    }
    ...
    public void handleNewUser(LocalDispatcher dispatcher) throws CartItemModifyException {
        ...
        if (UtilValidate.isNotEmpty(partyId)) {
            ...
            try {
                ...
                if (UtilValidate.isNotEmpty(getAEPartyRoles)) {
                    boolean isTrade = ((Boolean) getAEPartyRoles.get("isTrade")).booleanValue();
                    boolean isNonProfit = ((Boolean) getAEPartyRoles.get("isNonProfit")).booleanValue();
                    boolean isNonTaxable = ((Boolean) getAEPartyRoles.get("isNonTaxable")).booleanValue();
                    boolean isTradePostNet = ((Boolean) getAEPartyRoles.get("isTradePostNet")).booleanValue();
                    boolean isTradeAllegra = ((Boolean) getAEPartyRoles.get("isTradeAllegra")).booleanValue();
                    if (isTrade || isTradePostNet || isTradeAllegra) {
                        this.setTradeStatus(true);
                    } else {
                        this.setTradeStatus(false);
                    }

                    if (isNonProfit) {
                        this.setNonProfitStatus(true);
                    } else {
                        this.setNonProfitStatus(false);
                    }

                    if (isNonTaxable) {
                        this.setNonTaxableStatus(true);
                    } else {
                        this.setNonTaxableStatus(false);
                    }
                }
            } catch (GenericServiceException e) {
                Debug.logError(e, "Error trying to get trade roles for user.", module);
            }
            ...
            if(this.doPromotions) {
                //create copy of coupons entered
                Set<String> promoCodes = new HashSet<>();
                promoCodes.addAll(this.productPromoCodes);

                //remove all promotions
                this.removeProductPromoCode(null, dispatcher);

                //re-apply promotions
                Iterator<String> promoCodeIter = promoCodes.iterator();
                while (promoCodeIter.hasNext()) {
                    ...
                    if (checkResult == null) {
                        this.productPromoCodes.add(promoCode);
                    }
                }
   ...
   public Timestamp getDefaultShipAfterDate() {
    ...
    public void setOrderPartyId(String orderPartyId) {
        ...
        try {
            this.party = EntityQuery.use(this.getDelegator()).from("Party").where(UtilMisc.toMap("partyId", orderPartyId)).cache(true).queryOne();
        } catch (GenericEntityException e) {
            Debug.logError(e, "Error trying to get party", module);
        }
    }

    public boolean getTradeStatus() { return this.isTrade; }
    public void setTradeStatus(boolean isTrade) { this.isTrade = isTrade; }

    public boolean getNonProfitStatus() { return this.isNonProfit; }
    public void setNonProfitStatus(boolean isNonProfit) { this.isNonProfit = isNonProfit; }

    public boolean getNonTaxableStatus() { return this.isNonTaxable; }
    public void setNonTaxableStatus(boolean isNonTaxable) { this.isNonTaxable = isNonTaxable; }
    ...
    public GenericValue getParty() {
        GenericValue party = this.party;

        try {
            if (party == null && getUserLogin() != null) {
                party = EntityQuery.use(this.getDelegator()).from("Party").where("partyId", getUserLogin().getString("partyId")).queryOne();
                this.party = party;
            }
            if (party == null && getAutoUserLogin() != null) {
                party = EntityQuery.use(this.getDelegator()).from("Party").where("partyId", getAutoUserLogin().getString("partyId")).queryOne();
                this.party = party;
            }
        } catch(GenericEntityException e) {
            Debug.logError(e, "Error trying to get party.", module);
    ...
    public String getExternalPartyId() {
        GenericValue party = this.getParty();
    ...
    public void clear() {
        ...
        this.orderEmailContactMechId = null;
    ...
    */
    ...
    public void setOrderEmailContactMechId(String orderEmailContactMechId) {
        this.orderEmailContactMechId = orderEmailContactMechId;
    }

    public String getOrderEmailContactMechId() {
        return orderEmailContactMechId;
    }
    ...
    public void setDefaultCheckoutOptions(LocalDispatcher dispatcher) {
        ...
        if (getOrderType().equals("SALES_ORDER")) {
            ...
            if (this.getPartyId() != null && !this.getPartyId().equals("_NA_")) {
                GenericValue orderParty = this.getParty();
    ...
    // Method to remove product promo from shopping cart
    public void removeProductPromoCode(String productPromoCodeId, LocalDispatcher dispatcher) {
        // check all promo codes, remove any that are specified, else remove all
        Iterator<String> promoCodeIter = this.productPromoCodes.iterator();
        while (promoCodeIter.hasNext()) {
            String promoCode = promoCodeIter.next();
            if (productPromoCodeId == null || promoCode.equals(productPromoCodeId)) {
    ...
    public String addProductPromoCode(String productPromoCodeId, LocalDispatcher dispatcher) {
        ...
        if (checkResult == null) {
            ...
            BigDecimal oldOrderTotal = this.getDisplayGrandTotal();
            BigDecimal newOrderTotal = ProductPromoWorker.doPromotions(this, dispatcher);

            //if the order total didnt change, that means coupon didnt work, eject it
            //unless the coupon type was shipping, then apply it

            if(oldOrderTotal.compareTo(newOrderTotal) == 0) {
                boolean isShipPromo = false;
                BigDecimal shipRequirement = BigDecimal.ZERO;
                try {
                    isShipPromo = ProductPromoWorker.isProductPromoActionOfType(this.getDelegator(), productPromoCodeId, "PROMO_SHIP_CHARGE");
                    shipRequirement = ProductPromoWorker.getShippingRequirement(this.getDelegator(), productPromoCodeId);
                } catch(Exception e) {
                    Debug.logError(e, module);
                }

                if(isShipPromo && oldOrderTotal.compareTo(shipRequirement) < 0) {
                    this.removeProductPromoCode(productPromoCodeId, dispatcher);
                    return "Your cart total did not meet the requirements for free standard shipping.";
                } else if(isShipPromo) {
                    //do nothing if its just a ship promo and it needs to apply, they just didnt select shipment method yet
                } else {
                    this.removeProductPromoCode(productPromoCodeId, dispatcher);
                    return "This coupon cannot be applied to the items in your cart.";
                }
            }
            return null;
        } else {
        ...
        synchronized (cartLines) {
            ...
            for (ShoppingCartItem item : cartLines) {
                ...
                for (String key : envPriceCalcAttributes.keySet()) {
                    if(key.equals("isRush")) {
                        Boolean value = (Boolean) envPriceCalcAttributes.get(key);
                        item.setRushProduction((UtilValidate.isNotEmpty(value) && value) ? "Y" : "N");
                    }
                }

                Map<String, Object> envArtworkAttributes = (Map<String, Object>) item.getAttribute("envArtworkAttributes");
                for (String key : envArtworkAttributes.keySet()) {
                    if(key.equals("artworkSource")) {
                        String value = (String) envArtworkAttributes.get(key);
                        item.setArtworkSource(value);
                    } else if (key.equals("content")) {
                        List<Map> contentList = (List<Map>) envArtworkAttributes.get(key);
                        for (Map content : contentList) {
                            item.setReferenceOrderId((String) content.get("reOrderId"));
                            item.setReferenceOrderItemSeqId((String) content.get("reOrderItemSeqId"));
                        }
                    }
                }

                Map<String, Object> envQuoteAttributes = (Map<String, Object>) item.getAttribute("envQuoteAttributes");
                item.setReferenceQuoteId((String) envQuoteAttributes.get("quoteId"));
                item.setReferenceQuoteItemSeqId((String) envQuoteAttributes.get("quoteItemSeqId"));
                ...
                orderItem.set("isRushProduction", item.getRushProduction());
                orderItem.set("artworkSource", item.getArtworkSource());
                orderItem.set("referenceOrderId", item.getReferenceOrderId());
                orderItem.set("referenceOrderItemSeqId", item.getReferenceOrderItemSeqId());
                orderItem.set("referenceQuoteId", item.getReferenceQuoteId());
                orderItem.set("referenceQuoteItemSeqId", item.getReferenceQuoteItemSeqId());
    ...
    public List<GenericValue> makeAllOrderItemAttributes(String orderId, int mode) {
		...
		/*
                 * BELOW IS ENV SPECIFIC CODE, THIS SHOULD IDEALY BE DONE IN ITS OWN METHOD
                 * BUT IN AN EFFORT TO NOT MODIFY MORE CORE OFBIZ FILES WE WILL DO IT HERE
                 */
                Map<String, Object> envPriceCalcAttributes = (Map<String, Object>) item.getAttribute("envPriceCalcAttributes");
                for (String key : envPriceCalcAttributes.keySet()) {
                    String value = null;

                    if (envPriceCalcAttributes.get(key) instanceof String) {
                        value = (String) envPriceCalcAttributes.get(key);
                    } else if (envPriceCalcAttributes.get(key) instanceof Integer) {
                        value = ((Integer) envPriceCalcAttributes.get(key)).toString();
                    } else if (envPriceCalcAttributes.get(key) instanceof Boolean) {
                        value = String.valueOf((Boolean) envPriceCalcAttributes.get(key));
                    }

                    if (ALL == mode || (FILLED_ONLY == mode && UtilValidate.isNotEmpty(value)) || (EMPTY_ONLY == mode && UtilValidate.isEmpty(value))
                        || (mode != ALL && mode != FILLED_ONLY && mode != EMPTY_ONLY)) {

                        if(!key.equals("isRush") && !key.equals("id") && !key.equals("quantity") && !key.equals("name") && UtilValidate.isNotEmpty(value)) { //rush goes into orderitem
                            GenericValue orderItemAttribute = getDelegator().makeValue("OrderItemAttribute");
                            if (UtilValidate.isNotEmpty(orderId)) {
                                orderItemAttribute.set("orderId", orderId);
                            }
                ...
                Map<String, Object> envArtworkAttributes = (Map<String, Object>) item.getAttribute("envArtworkAttributes");
                if (UtilValidate.isNotEmpty(envArtworkAttributes) && isItemPrinted(item)) {
                    GenericValue orderItemArtwork = getDelegator().makeValue("OrderItemArtwork", UtilMisc.toMap("orderItemArtworkId", getDelegator().getNextSeqId("OrderItemArtwork")));
                    if (UtilValidate.isNotEmpty(orderId)) {
                        orderItemArtwork.set("orderId", orderId);
                    }
                    orderItemArtwork.set("orderItemSeqId", item.getOrderItemSeqId());
                    for (String key : envArtworkAttributes.keySet()) {
                        String value = null;
                        if (key.equals("content")) {
                            List<Map> contentList = (List<Map>) envArtworkAttributes.get(key);
                            for(Map content : contentList) {
                                GenericValue orderItemContent = getDelegator().makeValue("OrderItemContent", UtilMisc.toMap("orderItemContentId", getDelegator().getNextSeqId("OrderItemContent")));
                                if (UtilValidate.isNotEmpty(orderId)) {
                                    orderItemContent.set("orderId", orderId);
                                }
                                orderItemContent.set("orderItemSeqId", item.getOrderItemSeqId());
                                orderItemContent.set("contentPurposeEnumId", (String) content.get("contentPurposeEnumId"));
                                orderItemContent.set("contentPath", (String) content.get("contentPath"));
                                orderItemContent.set("contentName", (String) content.get("contentName"));
                                orderItemContent.set("fromDate", UtilDateTime.nowTimestamp());
                                result.add(orderItemContent);

                                item.setReferenceOrderId((String) content.get("reOrderId"));
                                item.setReferenceOrderItemSeqId((String) content.get("reOrderItemSeqId"));
                            }
                        } else if (!key.equals("artworkSource") && !key.equals("scene7ParentId") && !key.equals("isProduct") && !key.equals("designId") && !key.equals("token")) {
                            if (envArtworkAttributes.get(key) instanceof String) {
                                value = (String) envArtworkAttributes.get(key);
                            } else if (envArtworkAttributes.get(key) instanceof Integer) {
                                value = ((Integer) envArtworkAttributes.get(key)).toString();
                            } else if (envArtworkAttributes.get(key) instanceof Boolean) {
                                value = String.valueOf((Boolean) envArtworkAttributes.get(key));
                            }
                            orderItemArtwork.set(key, value);
                        }
                    }
                    result.add(orderItemArtwork);
                }
            }
            return result;
        }
    ...
    public boolean isItemPrinted(ShoppingCartItem item) {
        Map<String, Object> envPriceCalcAttributes = (Map<String, Object>) item.getAttribute("envPriceCalcAttributes");
        Integer colorsFront = (Integer) envPriceCalcAttributes.get("colorsFront");
        Integer colorsBack = (Integer) envPriceCalcAttributes.get("colorsBack");
        Boolean isFullBleed = (Boolean) envPriceCalcAttributes.get("isFullBleed");

        if(colorsFront == null) { colorsFront = 0; }
        if(colorsBack == null) { colorsBack = 0; }
        if(isFullBleed == null) { isFullBleed = false; }
        if(colorsFront > 0 || colorsBack > 0 || isFullBleed) {
            return true;
        }
    ...
    public BigDecimal numOfSamplesInCart() {
        BigDecimal items = BigDecimal.ZERO;
        List<ShoppingCartItem> cartLines = this.items();
        for(ShoppingCartItem sci : cartLines) {
            if(sci.getQuantity().compareTo(new BigDecimal("5")) <= 0) {
                items = items.add(sci.getQuantity());
            }
        }
            ...
            if ("PaymentMethod".equals(valueObj.getEntityName())) {
                ...
                try {
                    ...
                    if ("CREDIT_CARD".equals(paymentMethodTypeId)) {
                        ...
                        pmObj = EntityQuery.use(delegator).from("AmazonPaymentMethod").where("paymentMethodId", paymentMethodId).queryOne();
                    } else if ("EXT_PAYPAL_CHECKOUT".equals(paymentMethodTypeId)) {
                        pmObj = EntityQuery.use(delegator).from("PayPalCheckoutPaymentMethod").where("paymentMethodId", paymentMethodId).queryOne();
                    } else if ("EXT_OFFLINE".equals(paymentMethodTypeId) || "CERTIFIED_CHECK".equals(paymentMethodTypeId) || "PERSONAL_CHECK".equals(paymentMethodTypeId) || "EXT_IPARCEL".equals(paymentMethodTypeId) || "EXT_NET30".equals(paymentMethodTypeId) || "EXT_CLOSED".equals(paymentMethodTypeId)) {
                        pmObj = EntityQuery.use(delegator).from("OfflinePaymentMethod").where("paymentMethodId", paymentMethodId).queryOne();
                    }
                    if (pmObj != null) {
