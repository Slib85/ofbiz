...
public class PaymentGatewayServices {
    ...
    private static Map<String, Object> authPayment(LocalDispatcher dispatcher, GenericValue userLogin, OrderReadHelper orh, GenericValue paymentPreference, BigDecimal totalRemaining, boolean reauth, BigDecimal overrideAmount) throws GeneralException {
        ...
        processContext.put("salesChannelEnumId", orderHeader.getString("salesChannelEnumId"));
    ...
    private static String getBillingInformation(OrderReadHelper orh, GenericValue paymentPreference, Map<String, Object> toContext) throws GenericEntityException {
        ...
        if (paymentMethod != null && "CREDIT_CARD".equals(paymentMethodTypeId)) {
            ...
            GenericValue amazonPaymentMethod = paymentMethod.getRelatedOne("AmazonPaymentMethod", false);
            toContext.put("amazonPaymentMethod", amazonPaymentMethod);
        } else if ("EXT_PAYPAL_CHECKOUT".equals(paymentMethodTypeId)) {
            GenericValue payPalPaymentMethod = paymentMethod.getRelatedOne("PayPalCheckoutPaymentMethod", false);
            toContext.put("payPalPaymentMethod", payPalPaymentMethod);
        } else if ("EXT_OFFLINE".equals(paymentMethodTypeId) || "EXT_IPARCEL".equals(paymentMethodTypeId) || "EXT_NET30".equals(paymentMethodTypeId) || "EXT_CLOSED".equals(paymentMethodTypeId)) {
            GenericValue offlinePaymentMethod = paymentMethod.getRelatedOne("OfflinePaymentMethod", false);
            toContext.put("offlinePaymentMethod", offlinePaymentMethod);
        } else {
    ...
    public static Map<String, Object> releaseOrderPaymentPreference(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        // This was created due to how MES was implemented.  It has some pretty deep roots that we would have to carefully pick out to avoid failure
        // with our new CC Processor.  This may not be fully cleaned up due to the nature of moving to a new platform.
        if ((paymentSettings.get("paymentMethodTypeId")).equals("CREDIT_CARD") || (paymentSettings.get("paymentMethodTypeId")).equals("EXT_PAYPAL_CHECKOUT")) {
            return result;
        }
    ...
    public static Map<String, Object> processAuthResult(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        try {
            ...
            response.set("transactionDate", UtilDateTime.nowTimestamp());
            response.set("requestMsg", context.get("requestMsg"));
